...
import ActionsDropdown, { Action, DeleteDialog, PostHeader, PostTitle } from 'pages/Blog/Post/post.styled';
          ...
          (id !== 'new' && !post) ? 'loading...' : (
            <form onSubmit={handleSave} style={{display: 'flex', flexDirection: 'column', gap: '24px'}}>
              <PostTitle
                ...
                style={{border: (isReadOnly || user?.role !== USER_ROLES.admin) ? 'none' : '1px solid #ffffff80'}}
              />
...

              {
                !isReadOnly && (
                  <Box style={{display: 'flex', gap: '24px'}}>
                    {
                      id !== 'new' &&
                      <Button type="button" color="transparent" onClick={handleCancel}>Cancel</Button>
                    }
                    <Button type="submit" color="gradient">{id === 'new' ? 'Create' : 'Update'} post</Button>
                  </Box>
                )
              }
